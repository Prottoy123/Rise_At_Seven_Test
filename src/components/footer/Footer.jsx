      ...
      <div className="max-w-[1600px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8 mb-24">
          ...
          <div className="md:col-span-5 flex flex-col items-start">
            ...
            <div className="w-full max-w-[500px] relative mb-10 mt-4">
              <input 
                type="email" 
                placeholder="Your Email Address" 
                className="w-full bg-[#222222] text-white rounded-full py-6 pl-8 pr-20 text-lg outline-none border border-transparent focus:border-gray-500 transition-colors placeholder-gray-400 font-medium"
              />
              <button className="absolute right-3 top-1/2 -translate-y-1/2 w-12 h-12 bg-[#b2f9e1] text-black rounded-full flex items-center justify-center font-bold hover:bg-white transition-colors duration-300 group overflow-hidden">
                <div className="relative w-full h-full flex items-center justify-center">
                  <span className="absolute transition-transform duration-300 ease-in-out group-hover:translate-x-8 group-hover:-translate-y-8 text-lg">↗</span>
                  <span className="absolute transition-transform duration-300 ease-in-out -translate-x-8 translate-y-8 group-hover:translate-x-0 group-hover:translate-y-0 text-lg">↗</span>
                </div>
              </button>
            </div>

            <div className="flex gap-3">
              {[FaFacebookF, FaTwitter, FaLinkedinIn, FaYoutube, FaTiktok, FaInstagram].map((Icon, idx) => (
                <a key={idx} href="#" className="w-12 h-10 bg-white text-black rounded-full flex items-center justify-center hover:bg-[#b2f9e1] transition-colors duration-300 group px-3 relative overflow-hidden">
                  <Icon className="text-sm z-10" /> 
                  <div className="relative w-3 h-3 ml-1.5 overflow-hidden z-10">
                    <span className="absolute inset-0 flex items-center justify-center text-[10px] font-black transition-transform duration-300 ease-in-out group-hover:translate-x-4 group-hover:-translate-y-4">↗</span>
                    <span className="absolute inset-0 flex items-center justify-center text-[10px] font-black transition-transform duration-300 ease-in-out -translate-x-4 translate-y-4 group-hover:translate-x-0 group-hover:translate-y-0">↗</span>
                  </div>
                </a>
              ))}
            </div>
