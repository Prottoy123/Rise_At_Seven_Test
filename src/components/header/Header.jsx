...
const menuItems = [
  ...
  { name: "Blog & Resources", hasPlus: false },
...
const Header = () => {
  ...
  return (
    <div className="absolute top-0 left-0 w-full z-[100]">
      ...
      <div 
        className={`fixed inset-0 bg-white/20 backdrop-blur-md z-40 transition-opacity duration-300 lg:block hidden ${hoveredMenu ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}
      ></div>
      ...
      <header
        className={` left-0 right-0 z-50 transition-all duration-500 px-6 
        ${
          scrolled
            ? "top-4 mx-auto w-[95%] bg-white/80 backdrop-blur-xl border border-white/20 rounded-full py-3 shadow-2xl"
            : "top-12 w-full bg-transparent py-6"
        }`}
      >
        <div className="lg:container w-full  mx-auto flex items-center justify-between">
          {/* Logo Section */}
          <div className={`w-32 md:w-40 transition-colors duration-300 ${scrolled ? "text-black" : "text-white"}`}>
            <svg
          ...
          <nav className={`hidden lg:flex items-center space-x-6 text-[13px] font-bold uppercase tracking-tight transition-colors duration-300 ${scrolled ? "text-black" : "text-white"}`}>
            {menuItems.map((item) => (
              <div
                key={item.name}
                className="relative cursor-pointer py-2 group"
                onMouseEnter={() => item.hasMega && setHoveredMenu(item.name)}
                onMouseLeave={() => item.hasMega && setHoveredMenu(null)}
              >
                <div className="flex items-center gap-1">
                   <span className={`px-2 py-1 transition-all rounded-full ${scrolled ? "hover:bg-black hover:text-white" : "hover:bg-white hover:text-black"} ${hoveredMenu === item.name ? "bg-white text-black" : ""}`}>
                    {`${item.name}${item.hasPlus ? " +" : ""}`}
                   </span>
                  {item.hasBadge && (
                    <span className="bg-[#b2f9e1] text-[10px] text-black px-1.5 py-0.5 rounded-full font-black">
                      {item.badgeCount}
                    </span>
                  )}
                </div>
                ...
                {item.hasMega && (
                  <div
                    className={`absolute left-1/2 -translate-x-1/2 top-full mt-2 w-[850px] bg-white rounded-[32px] p-8 shadow-2xl text-black transition-all duration-300 transform origin-top cursor-default z-[60] ${
                      hoveredMenu === item.name ? "opacity-100 scale-100 visible" : "opacity-0 scale-95 invisible pointer-events-none"
                    }`}
                  >
                    <div className="grid grid-cols-12 gap-8 text-left">
                      <div className="col-span-8">
                        <p className="text-xs uppercase font-bold text-gray-400 mb-6">{item.name === "Services" ? "Core Services" : `Explore ${item.name}`}</p>
                        <div className="grid grid-cols-2 gap-y-3 gap-x-6 normal-case text-[15px] font-bold text-gray-900">
                          {coreServices.map((service) => (
                            <a key={service} href="#" className="block">
                              <AnimatedLink name={service} />
                            </a>
                          ))}
                        </div>
                      </div>
                      <div className="col-span-4 relative rounded-2xl overflow-hidden bg-gray-100 flex flex-col justify-end p-5 group/card">
                        <img
                        ...
                        <div className="relative z-10 text-center w-full">
                          <p className="text-white font-black text-lg mb-3">The Rise Lounge</p>
                          <button className="bg-black text-white text-[10px] font-black px-4 py-2 rounded-full uppercase hover:scale-105 transition-transform">
          ...
          <div className="flex items-center gap-4">
            <button
              className={`hidden md:block px-6 py-2.5 rounded-full text-xs font-black transition-all duration-300 ${
                scrolled ? "bg-black text-white" : "bg-white text-black hover:scale-105"
              }`}
            >
              <AnimatedLink name="Get In Touch ↗" />
            </button>
            ...
            >
              <div className={`w-8 h-[2px] transition-colors ${scrolled ? "bg-black" : "bg-white"}`}></div>
              <div className={`w-6 h-[2px] transition-colors ${scrolled ? "bg-black" : "bg-white"}`}></div>
            </button>
          </div>
        </div>
      </header>
      ...
      >
        <div className="w-full h-full bg-[#1a1c1e] flex flex-col p-8 overflow-y-auto">
          <div className="flex justify-between items-center mb-10">
            <span className="text-white text-xl font-black tracking-tighter">Rise at Seven.</span>
            <button
          ...
          <nav className="flex flex-col space-y-4">
            {["Services", "Industries", "International", "About", "Work", "Careers", "Blog & Resources", "Webinar"].map((name) => (
              <div key={name} className="flex items-center justify-between py-1">
                <a href="#" className="text-[32px] font-black text-white tracking-tighter hover:text-[#b2f9e1] transition-colors leading-none">
                  {name}
                </a>
            ...
            <div className="pt-8 mt-auto">
              <button className="w-full bg-white text-black py-4 rounded-full font-bold text-base hover:bg-gray-200 transition-colors">
                Get In Touch ↗
              </button>
            </div>
          </nav>
