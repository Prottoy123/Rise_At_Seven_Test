...
const menuItems = [
  { name: "Services", hasPlus: true, hasMega: true },
  { name: "Industries", hasPlus: true, hasMega: true },
  { name: "International", hasPlus: true, hasMega: true },
  { name: "About", hasPlus: true, hasMega: true },
  { name: "Work", hasBadge: true, badgeCount: 25 },
  { name: "Careers", hasPlus: false },
  { name: "Blog & Resources", hasPlus: true, hasMega: true },
...
const megaMenuData = {
  "Services": {
    ...
    links: [
      "Search & Growth Strategy", "Digital PR", "Onsite SEO", 
      "Social Media & Campaigns", "Content Experience", 
      "Data & Insights", "B2B Marketing", "Social SEO/Search"
    ],
    gridClass: "grid-cols-2 gap-x-12",
    linkClass: "text-[22px]",
    imgCardClass: "w-80",
    imgSrc: "https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&w=600&q=80",
    imgTitle: "View all services",
    showIcon: true
  },
  "Industries": {
    heading: "",
    links: ["B2B Marketing", "eCommerce", "Finance", "Healthcare"],
    gridClass: "grid-cols-1",
    linkClass: "text-3xl",
    imgCardClass: "w-72",
    imgSrc: "https://images.unsplash.com/photo-1542744173-8e7e53415bb0?auto=format&fit=crop&w=600&q=80",
    imgTitle: "Industry Insights"
  },
  "International": {
    heading: "",
    links: ["US Digital PR", "Spain Digital PR", "Germany Digital PR"],
    gridClass: "grid-cols-1",
    linkClass: "text-3xl",
    imgCardClass: "w-72",
    imgSrc: "https://images.unsplash.com/photo-1556761175-5973dc0f32e7?auto=format&fit=crop&w=600&q=80",
    imgTitle: "Global Reach"
  },
  "About": {
    heading: "",
    links: ["About Us", "Meet the Team", "Our Culture"],
    gridClass: "grid-cols-1",
    linkClass: "text-3xl",
    imgCardClass: "w-72",
    imgSrc: "https://images.unsplash.com/photo-1517048676732-d65bc937f952?auto=format&fit=crop&w=600&q=80",
    imgTitle: "Inside Rise"
  },
  "Blog & Resources": {
    heading: "",
    links: ["The Search-First Guide", "SEO Insights", "Digital PR News", "Case Studies"],
    gridClass: "grid-cols-1",
    linkClass: "text-3xl",
    imgCardClass: "w-72",
    imgSrc: "https://images.unsplash.com/photo-1499750310107-5fef28a66643?auto=format&fit=crop&w=600&q=80",
    imgTitle: "Latest Thinking"
  }
};

const AnimatedLink = ({ name, className = "" }) => (
  <span className={`relative overflow-hidden inline-flex group/link ${className}`}>
    <span className="block transition-transform duration-300 ease-in-out group-hover/link:-translate-y-[120%]">{name}</span>
    <span className="absolute top-0 left-0 w-full translate-y-[120%] transition-transform duration-300 ease-in-out group-hover/link:-translate-y-0">{name}</span>
  </span>
);
...
const Header = () => {
  ...
  const currentMenu = hoveredMenu ? megaMenuData[hoveredMenu] : null;

  return (
    <div className="absolute top-0 left-0 w-full z-[100]">
      {/* Background Blur Overlay for Desktop Menu */}
      <div 
        className={`fixed inset-0 bg-white/40 backdrop-blur-md z-40 transition-opacity duration-300 lg:block hidden ${hoveredMenu ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}
      ></div>
      ...
      <header className="absolute left-0 right-0 z-50 top-12 w-full bg-transparent py-6 px-4 md:px-6">
        <div className="lg:container w-full mx-auto flex items-center justify-between relative">
          ...
          <nav 
            className="hidden lg:flex items-center space-x-2 xl:space-x-4 text-[13px] font-bold uppercase tracking-tight text-white transition-colors duration-300"
          ...
          >
            {menuItems.map((item) => (
              <div
                ...
                onMouseEnter={() => item.hasMega && setHoveredMenu(item.name)}
              >
                <div className="flex items-center gap-1">
                   <span className={`px-3 py-2 transition-all rounded-full hover:bg-white hover:text-black ${hoveredMenu === item.name ? "bg-white text-black" : ""}`}>
                    {`${item.name}${item.hasPlus ? " +" : ""}`}
                   </span>
                  {item.hasBadge && (
                    <span className="bg-[#b2f9e1] text-[10px] text-black px-2 py-0.5 rounded-full font-black -ml-2 z-10">
                      {item.badgeCount}
                    </span>
                  )}
                </div>
          ...
          <div className="flex items-center gap-4">
            <button
              className="hidden md:block px-6 py-3 rounded-full text-[13px] uppercase font-black transition-all duration-300 bg-white text-black hover:scale-105"
            >
              <AnimatedLink name="Get In Touch ↗" />
            </button>
            <button
              onClick={() => setIsMenuOpen(true)}
              className="lg:hidden flex flex-col gap-2 p-2 relative group items-end"
            >
              <div className="w-8 h-[2px] bg-white transition-colors"></div>
              <div className="w-6 h-[2px] bg-white transition-colors"></div>
            </button>
          </div>

          {/* GLOBAL DYNAMIC MEGA MENU CONTAINER (Hugs content perfectly) */}
          <div
            className={`absolute left-1/2 -translate-x-1/2 top-full mt-4 bg-white rounded-[32px] p-3 flex shrink-0 shadow-2xl text-black transition-all duration-300 transform origin-top cursor-default z-[60] ${
              hoveredMenu ? "opacity-100 scale-100 visible" : "opacity-0 scale-95 invisible pointer-events-none"
            }`}
          ...
          >
            {currentMenu && (
              <>
                    ...
                    <div className="flex-1">
                      {currentMenu.heading && (
                        <div className="h-8 mb-2">
                          <p className="text-[13px] tracking-tight font-medium text-gray-400">
                            {currentMenu.heading}
                          </p>
                        </div>
                      )}
                      <div className={`grid ${currentMenu.gridClass} gap-y-2 font-medium`}>
                        {currentMenu.links.map((link) => (
                          <a key={link} href="#" className={`block hover:text-gray-500 transition-colors ${currentMenu.linkClass} tracking-tight leading-tight`}>
                            <AnimatedLink name={link} />
                          </a>
                        ))}
                      </div>
                    </div>
                ...
                <div className={`shrink-0 relative ${currentMenu.imgCardClass}`}>
                  <div className="relative rounded-2xl overflow-hidden bg-black aspect-square w-full shrink-0 group/card">
                    <img
                      src={currentMenu.imgSrc}
                      alt={currentMenu.imgTitle}
                      className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover/card:scale-105"
                    />
                    <div className="absolute inset-0 bg-black/20"></div>
                    {currentMenu.showIcon ? (
                       <a href="#" className="absolute bottom-5 left-5 right-5 group/btn inline-flex justify-center gap-x-2 items-center bg-black text-white px-6 py-3 rounded-3xl hover:rounded-xl transition-all duration-300">
                         <span className="text-[13px] font-medium"><AnimatedLink name={currentMenu.imgTitle} /></span>
                       </a>
                    ) : (
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <span className="text-white text-xl font-bold tracking-tight bg-black/50 px-4 py-2 rounded-full backdrop-blur-sm">
                          {currentMenu.imgTitle}
                        </span>
                      </div>
                    )}
                  </div>
      ...
      <div
        className={`fixed inset-0 z-[200] transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] transform ${
          isMenuOpen ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"
        }`}
      >
        <div className="w-full h-full bg-[#111111] flex flex-col px-6 py-6 md:px-12 md:py-10 overflow-y-auto">
          <div className="flex justify-between items-center mb-10 shrink-0">
            <svg className="h-6 md:h-8 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 168 21">
              <path d="M91.3152 5.40061C91.3152 3.94241 92.5306 2.67359 93.9881 2.67359C95.7162 2.67359 96.797 3.83419 96.797 5.56225H99.7127C99.7127 2.1873 97.3096 0 93.9874 0C90.9371 0 88.3988 2.32257 88.3988 5.42766C88.3988 9.31596 90.883 10.2344 93.9874 11.4221C95.6627 12.07 97.2007 12.5563 97.2007 14.6895C97.2007 16.634 95.9867 18.0651 93.9874 18.0651C91.8813 18.0651 90.7477 16.3905 90.7477 14.446H87.832C87.832 18.0651 90.3426 20.7381 93.9874 20.7381C97.6323 20.7381 100.118 18.2816 100.118 14.6895C100.118 7.10161 91.3145 9.64061 91.3145 5.40061H91.3152Z"></path>
              <path d="M109.209 4.99609C104.834 4.99609 101.539 8.53405 101.539 12.8539C101.539 17.1737 104.888 20.738 109.155 20.738C112.422 20.738 115.203 18.713 116.337 15.662H113.529C112.718 17.2278 111.017 18.1733 109.262 18.1733C106.806 18.1733 104.915 16.4182 104.348 14.0963H116.743C116.797 13.6371 116.823 13.1508 116.823 12.6922C116.823 8.47926 113.447 4.99609 109.209 4.99609ZM104.348 11.9361C104.509 9.47823 106.751 7.56147 109.181 7.56147C111.611 7.56147 113.853 9.47823 114.014 11.9361H104.348Z"></path>
              <path d="M127.476 5.40039L123.575 16.0941L119.673 5.40039H116.676L122.617 20.3598H124.588L130.475 5.40039H127.476Z"></path>
              <path d="M137.942 4.99609C133.567 4.99609 130.273 8.53405 130.273 12.8539C130.273 17.1737 133.621 20.738 137.888 20.738C141.155 20.738 143.936 18.713 145.071 15.662H142.262C141.453 17.2278 139.75 18.1733 137.996 18.1733C135.538 18.1733 133.649 16.4182 133.081 14.0963H145.476C145.53 13.6371 145.556 13.1508 145.556 12.6922C145.556 8.47926 142.182 4.99609 137.942 4.99609ZM133.081 11.9361C133.243 9.47823 135.484 7.56147 137.915 7.56147C140.347 7.56147 142.586 9.47823 142.749 11.9361H133.081Z"></path>
          ...
          <nav className="flex flex-col space-y-0">
            {["Services", "Industries", "International", "About", "Work", "Careers", "Blog & Resources", "Webinar"].map((name) => (
              <div key={name} className="w-full border-b border-white/20 pb-4 pt-4 first:border-t flex items-center justify-between group cursor-pointer">
                <a href="#" className="text-3xl md:text-5xl tracking-tight font-medium leading-none text-white transition-colors group-hover:text-gray-300">
                  {name}
                </a>
                {["Services", "Industries", "International", "About", "Blog & Resources"].includes(name) && (
                  <button className="w-6 h-6 rounded-full border border-white/20 flex items-center justify-center text-white transition-colors group-hover:bg-white/10">
                    <ChevronDown size={14} />
                  </button>
                )}
              </div>
          ...
          <div className="pt-12 mt-auto">
            <button className="w-full bg-white text-black py-4 rounded-3xl font-medium text-lg leading-tight tracking-tightish hover:rounded-xl transition-all duration-300 flex items-center justify-center gap-x-2 group">
